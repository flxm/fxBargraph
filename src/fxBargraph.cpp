...
bool fxBargraph::setValue(int newval) {
  newval = constrain(newval, 0, maxval);

  if (newval == lastval) return false;
...
bool fxBargraph::draw() {
  int normalized = (((long)steps*width) * val / maxval );
  byte num =  normalized / steps;
  byte frac = normalized % steps;

  if (frac == lastfrac && num && lastnum) return false;

  if (frac != lastfrac && frac > 0) createFraction(frac);
